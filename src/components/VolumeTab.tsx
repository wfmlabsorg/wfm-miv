    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">
          <TrendingUp className="w-5 h-5 text-blue-600 inline mr-2" />
          Volume Forecast MIV
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-gray-50 p-4 rounded-lg border-l-4 border-blue-500">
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
            />
          </div>
          <div className="bg-gray-50 p-4 rounded-lg border-l-4 border-blue-500">
            <label className="block text-sm font-semibold text-gray-700 mb-1">
        ...
        {/* Day Distribution */}
        <h3 className="font-semibold text-gray-700 mb-2">Weekly Volume Distribution</h3>
            ...
            <tbody>
              {days.map((d, i) => (
                <tr key={d.name} className="border-b border-gray-100">
                  <td className="py-2 pr-4 text-sm font-medium text-gray-700">{d.name}</td>
                  <td className="py-2 pr-4">
                    <input
                      ...
                      className="w-20 border border-gray-300 rounded-md px-2 py-1 text-center text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                    />
                  </td>
                  <td className="py-2">
        ...
        {/* Hours of Operation */}
        <h3 className="font-semibold text-gray-700 mb-2">Hours of Operation</h3>
        <p className="text-sm text-gray-500 mb-3">
        ...
        <div className="space-y-3">
          {days.map((d, i) => (
            <div key={d.name} className="grid grid-cols-[100px_1fr_50px] items-center gap-4">
              ...
              />
              <span className="text-sm text-blue-600 font-semibold text-right">{d.hours}h</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl shadow-md p-5 border-l-4 border-blue-500">
          <div className="text-sm text-gray-500 mb-1">Weighted Minimal Interval Variance</div>
          <div className="text-3xl font-bold text-blue-600">
            {(result.weightedMIV * 100).toFixed(2)}%
          </div>
        </div>
        <div className={`bg-white rounded-xl shadow-md p-5 border-l-4 ${goalMet ? 'border-green-500' : 'border-red-500'}`}>
          <div className="text-sm text-gray-500 mb-1">Target vs Goal</div>
      ...
      {/* Chart */}
      <div className="bg-white rounded-xl shadow-md p-6">
        <h3 className="font-semibold text-gray-800 mb-4">MIV by Day of Week</h3>
        <ErrorBoundary>
          <VolumeChart dayResults={result.dayResults} forecastGoal={result.forecastGoal} />
        </ErrorBoundary>
      </div>

      {/* Detail Table */}
      <div className="bg-white rounded-xl shadow-md p-6">
        <h3 className="font-semibold text-gray-800 mb-4">Detailed Breakdown</h3>
            ...
            <tbody>
              {result.dayResults.filter(r => r.dailyCalls > 0).map(r => (
                <tr key={r.day} className="border-b border-gray-100">
                  <td className="py-2 pr-4 text-sm font-medium text-gray-700">{r.day}</td>
                  <td className="py-2 pr-4 text-sm text-gray-600">{Math.round(r.dailyCalls).toLocaleString()}</td>
                  <td className="py-2 pr-4 text-sm text-gray-600">{r.intervals}</td>
                  <td className="py-2 pr-4 text-sm text-gray-600">{r.callsPerInterval.toFixed(1)}</td>
                  <td className={`py-2 text-sm font-semibold ${r.mivPct > result.forecastGoal ? 'text-red-600' : 'text-green-600'}`}>
...
function StatCard({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: string; color: string }) {
  ...
  return (
    <div className={`bg-white p-4 rounded-lg shadow-md border-l-4 ${borderColors[color]} transition-all hover:shadow-lg`}>
